...
기존 direct provider 방식은 기본값으로 유지. 새 방식은
OPENCODE_ANYCLI_PROVIDER=openai-compat (또는 wrapper flag --provider openai-compat)
로만 활성화되는 experimental opt-in이다.
...
- OPENCODE_ANYCLI_PROVIDER=openai-compat opencode-anycli 실행 시 opencode가
  로컬 façade에 붙고, direct 모드는 아무 플래그 없이 기존 그대로 동작.

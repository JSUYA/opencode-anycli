...
describe("runOnce", () => {
  ...
  it("surfaces cline tool activity for readFile events as a textual marker", async () => {
    const out = [
  ...
  it("emits a structured `read` tool-call/result pair for cline readFile events", async () => {
    const out = [
      '{"type":"say","say":"tool","text":"{\\"tool\\":\\"readFile\\",\\"path\\":\\"docs/configuration.md\\",\\"content\\":\\"/repo/docs/configuration.md\\",\\"readLineStart\\":1,\\"readLineEnd\\":32}","partial":false}',
    ]
    ...
    expect(results[0]!.result).toMatchObject({ metadata: { loaded: ["/repo/docs/configuration.md"] } })
  })

  it("does not emit a duplicate read tool-call when cline emits partial then final readFile events", async () => {
    const out = [
